...
export default function DocumentImport({ route, navigation }) {
  ...
  const processExcelData = async (jsonData) => {
    try {
      ...
      // Transform the JSON data to match the desired format
      const transformedData = jsonData.map((student) => {
        const transformedStudent = {};
        for (const [originalKey, newKey] of Object.entries(keyMapping)) {
          if (student[originalKey] !== undefined) {
            transformedStudent[newKey] = student[originalKey];
          }
        }
        transformedStudent.section = sectionId; // Add the sectionId to each student
        transformedStudent.createdAt = new Date().toISOString().split('T')[0]; // Add the createdAt field
      ...
      const newStudents = transformedData.filter((student) => !existingEmails.includes(student.email));
