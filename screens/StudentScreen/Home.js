...
import { StyleSheet, View, Alert, Dimensions, TouchableOpacity, FlatList, Modal, Image, ActivityIndicator, BackHandler } from 'react-native';
...
export default function Home({ route }) {
  ...
  useEffect(() => {
    ...
    const fetchSettings = async () => {
      ...
      if (docSnap.exists()) {
        const data = docSnap.data();
        setSettings(data.thresholds || { warning: 3, communityService: 5 });
  ...
  const checkEligibility = () => {
    ...
    if (
      ...
      if (tally >= settings.communityService) {
        Alert.alert(
          'Eligibility Check',
          `Today is the deadline.\n\nYour tally is ${tally}.\n\nYou are eligible for a reward.`
        );
      } else if (tally < settings.warning) {
        Alert.alert(
          'Eligibility Check',
          `Today is the deadline.\n\nYour tally is ${tally}.\n\nYou are eligible for a punishment.`
        );
      } else {
        Alert.alert(
          'Eligibility Check',
      ...
      Alert.alert(
        'Eligibility Check',
        `Today is not the deadline. The deadline is on ${deadlineDate.toDateString()}.`
      );
    }
  };
    ...
    <View style={styles.container}>
      <Card style={styles.card}>
        <Card.Content>
          ...
          <Button mode="contained" onPress={checkEligibility} style={styles.button}>
            Check Eligibility
          </Button>
